...
from updated_gimic_package.microbiome2matrix import micro2matrix
...
def creare_tree_view(folder,names, mean_0, mean_1, directory, threshold_p=0.05, family_colors=None):
    """
    ...
    """
    T = ete3.PhyloTree()
    u_test = pd.read_pickle(os.path.join(folder,"u_test_without_mimic.pkl"))
    flag_check_exist = False
    try:
        mimic_and_utest = pd.read_pickle(os.path.join(folder,"miMic&Utest.pkl"))
        ...
        for s in g.succ[node]:
            ...
            if s[0][-1] not in T or not any([anc.species == a for anc, a in
                ...
                if flag_check_exist:
                    ...
                    if len(mimic_and_utest_index) > 0: #mimic_and_utest_index != None or
    ...
    T0.render(os.path.join(directory,"correlations_tree.svg"), tree_style=deepcopy(ts))
...
def build_interactions(folder,bact_names, img_array, save, family_colors, threshold_p=0.05, THRESHOLD=0.5):
    """
    ...
    """
    all_ps = pd.read_pickle(os.path.join(folder,"df_corrs.pkl"))
    ...
    fig.savefig(os.path.join(save,"interaction.png"), dpi=300)
...
def calculate_all_imgs_tag_corr(samba_output, folder, tag, start_i, eval="corr", sis='fdr_bh', correct_first=False,
    ...
    if mode == "plot":
        if shuffle == False:
            ...
            directory= os.path.join(folder,'plots')
            ...
            df_corss = pd.read_pickle(os.path.join(folder,"df_corrs.pkl"))
        ...
        if not dict_pos and not dict_neg:
            ...
            plt.tight_layout()
            plt.savefig(os.path.join(directory,"corrs_within_family.png"), dpi=300)
            plt.show(block=False)

            # Interactions plot (5)
            build_interactions(folder,bact_names_df, img_arrays, directory, family_colors, threshold_p=threshold_p,
            ...
            creare_tree_view(folder,bact_names, imgs_p, imgs_s, directory, threshold_p, family_colors)
...
def plot_rp_sp_anova_p(df, mixed, save):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save,"tax_vs_rp_sp_anova_p.png"), dpi=300)
    plt.show(block=False)
...
def calculate_rsp(df, save):
    ...
    plt.yticks(fontsize=SIZE)
    plt.tight_layout()
    plt.savefig(os.path.join(save,"rsp_vs_beta.png"), dpi=300)
    plt.show(block=False)
...
def apply_mimic(folder, tag, eval="man", sis="fdr_bh", correct_first=True, mode="test", save=True, tax=None,
    ...
    with warnings.catch_warnings():
        ...
        if mode == 'test':
            ...
            if apply_samba:
                array_of_imgs, bact_names, ordered_df = micro2matrix(processed, folder, save=save)
            ...
            if t1 != 'noAnova':
                ...
                df_corrs_leaves_difference.to_pickle(os.path.join(folder,"u_test_without_mimic.pkl"))
                df_corrs.to_pickle(os.path.join(folder,"df_corrs.pkl"))
                mimic_and_utest.to_pickle(os.path.join(folder,"miMic&Utest.pkl"))

                # save statistic and p-values df
                if save:
                    df_corrs.to_csv(os.path.join(folder,"df_corrs.csv"))
                    df_corrs_leaves_difference.to_csv(os.path.join(folder,"u_test_without_mimic.csv"))
                    df_corrs123.to_csv(os.path.join(folder,"just_mimic.csv"))
                    mimic_and_utest.to_csv(os.path.join(folder,"miMic&Utest.csv"))

            else:
                df_corrs_leaves.to_pickle(os.path.join(folder,"u_test_without_mimic.pkl"))
                df_corrs_leaves.to_pickle(os.path.join(folder,"df_corrs.pkl"))

                if save:
                    df_corrs_leaves.to_csv(os.path.join(folder,"u_test_without_mimic.csv"))
            ...
            if tax != 'nosignificant':
                ...
                print('hihihihhihhihihihidirectory is:', directory)
                print('higigiggi folder is:', folder)
                plot_rp_sp_anova_p(results_to_plot, mixed, save=directory)
                # Plot RSP(beta) vs beta
                calculate_rsp(num_s_df, directory)
